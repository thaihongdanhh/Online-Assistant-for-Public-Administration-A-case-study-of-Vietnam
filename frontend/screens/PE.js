  ...
  const onSend = async (newMessages = []) => {    
    ...
    try{
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
          model: "ft:gpt-3.5-turbo-0613:data-science-lab::833NLm53",
      ...
      .then(async (response) => {
        ...
        const botMessage = {
          _id: new Date().getTime() + 1,
          text: data.choices[0].message.content.replace('OpenAI GPT-3 model','Online Assistant for E-Gorvernment'),
          createdAt: new Date(),
          user: {
            _id: 2,
            name: 'Online Assistant'
      ...
      fetch("http://ai.ailab.vn:5015/model/parse", requestOptions)
      .then(async (response) => {
        // res.send();        

        const data = await response.json()
        // console.log(data)
        console.log(data.response_selector.default.response.responses[0]['text'])
        const botMessage = {
          _id: new Date().getTime() + 1,
          text: data.response_selector.default.response.responses[0]['text'],
          createdAt: new Date(),
          user: {
            _id: 2,
            name: 'Online Assistant'
