"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.checkFullCapacity = checkFullCapacity;
exports.init = exports.createGlobalFunctionsInWebviewContext = void 0;
exports.navigateToLoginPage = navigateToLoginPage;
exports.postMessage = postMessage;
exports.postStreamedMessage = postStreamedMessage;
exports.reloadWebView = reloadWebView;
exports.removeThemeSwitcher = removeThemeSwitcher;
exports.retryLogin = retryLogin;
var _reactNativeUuid = _interopRequireDefault(require("react-native-uuid"));
var _types = require("../types");
var _constants = require("../constants");
var _parseStreamedGptResponse = _interopRequireDefault(require("../utils/parseStreamedGptResponse"));
var _getChatGptConversationHeaders = _interopRequireDefault(require("../utils/getChatGptConversationHeaders"));
var _wait = _interopRequireDefault(require("../utils/wait"));
var _httpCodes = require("../utils/httpCodes");
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
let webview;
const init = webviewRef => {
  webview = webviewRef;
};

/**
 * Monkey patches fetch to intercept ChatGPT requests and read the JWT
 * It also injects 2 methods in the global scope to accomplish the following:
 * 1. Sending messages to the ChatGPT backend directly from the Webview and stream the response back to RN
 * 2. Removing the theme switcher button from the webview when GPT shows it's at full capacity
 *
 * Note: It'd be cool to define the function in normal JS and
 * use fn.toString() or`${fn}` and wrap it in a IIFE,
 * but babel messes up the transformations of async/await and breaks the injected code.
 */
exports.init = init;
const createGlobalFunctionsInWebviewContext = () => {
  return `
    const { fetch: originalFetch } = window;
    window.fetch = async (...args) => {
      const [resource, config] = args;
      window.ReactNativeWebView.postMessage(JSON.stringify({type: 'REQUEST_INTERCEPTED_CONFIG', payload: config}));
      const response = await originalFetch(resource, config);
      return response;
    };

    window.removeThemeSwitcher = () => {
      const svgIcon = document.querySelector("button > svg");
      if (!svgIcon) {
        return;
      }
      const themeSwitchButton = svgIcon.closest('button');
      if (themeSwitchButton) {
        themeSwitchButton.style.display = 'none';
      }
    };

    window.sendGptMessage = async ({
      accessToken,
      message,
      messageId,
      newMessageId,
      conversationId,
      timeout
    }) => {

      async function* streamAsyncIterable(stream) {
        const reader = stream.getReader()
        try {
          while (true) {
            const { done, value } = await reader.read()
            if (done) {
              return
            }
            yield value
          }
        } finally {
          reader.releaseLock()
        }
      }

      function getHeaders(accessToken) {
        return {
          accept: "text/event-stream",
          "x-openai-assistant-app-id": "",
          authorization: accessToken,
          "content-type": "application/json",
          origin: "${_constants.HOST_URL}",
          referrer: "${_constants.CHAT_PAGE}",
          "sec-fetch-mode": "cors",
          "sec-fetch-site": "same-origin",
          "x-requested-with": "com.chatgpt3auth"
        };
      }

      const url = "${_constants.PROMPT_ENDPOINT}";
      const body = {
        action: "next",
        messages: [
          {
            id: newMessageId,
            role: "user",
            content: {
              content_type: "text",
              parts: [message],
            },
          },
        ],
        model: "text-davinci-002-render",
        parent_message_id: messageId,
      };

      if (conversationId) {
        body.conversation_id = conversationId;
      }

      const headers = getHeaders(accessToken);

      try {

        const controller = new AbortController();

        const timeoutId = setTimeout(() => {
          // Notifying RN that the request timed out
          window.ReactNativeWebView.postMessage(JSON.stringify({type: 'STREAM_ERROR', payload: {status: 408, statusText: 'Request timed out'}}));
          controller.abort();
        }, timeout);

        const res = await fetch(url, {
          method: "POST",
          body: JSON.stringify(body),
          headers: headers,
          mode: "cors",
          credentials: "include",
          signal: controller.signal
        });

        clearTimeout(timeoutId);

        if (res.status >= 400 && res.status < 600) {
          window.ReactNativeWebView.postMessage(JSON.stringify({type: 'STREAM_ERROR', payload: {status: res.status}}));
          return true;
        }

        for await (const chunk of streamAsyncIterable(res.body)) {
          const str = new TextDecoder().decode(chunk);
          window.ReactNativeWebView.postMessage(JSON.stringify({type: 'RAW_ACCUMULATED_RESPONSE', payload: str}));
        }
      } catch (e) {
        // Nothing to do here
      }
    };

    true;
  `;
};

/**
 * Calls a global function in the Webview window object to send a streamed message
 */
exports.createGlobalFunctionsInWebviewContext = createGlobalFunctionsInWebviewContext;
function postStreamedMessage(_ref) {
  var _webview;
  let {
    accessToken,
    message,
    messageId = _reactNativeUuid.default.v4(),
    conversationId,
    timeout = _constants.STREAMED_REQUEST_DEFAULT_TIMEOUT
  } = _ref;
  const newMessageId = _reactNativeUuid.default.v4();
  let script = '';
  if (conversationId) {
    script = `
      window.sendGptMessage({
        accessToken: "${accessToken}",
        message: "${message}",
        messageId: "${messageId}",
        newMessageId: "${newMessageId}",
        conversationId: "${conversationId}",
        timeout: ${timeout}
      });

      true;
    `;
  } else {
    script = `
      window.sendGptMessage({
        accessToken: "${accessToken}",
        message: "${message}",
        messageId: "${messageId}",
        newMessageId: "${newMessageId}",
        timeout: ${timeout}
      });

      true;
    `;
  }
  (_webview = webview) === null || _webview === void 0 ? void 0 : _webview.injectJavaScript(script);
}

/**
 * Sends a normal message to the ChatGPT conversation backend endpoint
 */
async function postMessage(_ref2) {
  let {
    accessToken,
    message,
    messageId = _reactNativeUuid.default.v4(),
    conversationId,
    timeout = _constants.REQUEST_DEFAULT_TIMEOUT,
    onTokenExpired
  } = _ref2;
  const controller = new AbortController();
  const newMessageId = _reactNativeUuid.default.v4();
  const timeoutId = setTimeout(() => {
    controller.abort();
    const error = new _types.ChatGptError('ChatGPTResponseClientError: Request timed out');
    error.statusCode = 408;
    throw error;
  }, timeout);
  const url = _constants.PROMPT_ENDPOINT;
  const body = {
    action: 'next',
    messages: [{
      id: newMessageId,
      role: 'user',
      content: {
        content_type: 'text',
        parts: [message]
      }
    }],
    model: 'text-davinci-002-render',
    parent_message_id: messageId
  };
  if (conversationId) {
    // @ts-ignore
    body.conversation_id = conversationId;
  }
  const res = await fetch(url, {
    method: 'POST',
    body: JSON.stringify(body),
    headers: (0, _getChatGptConversationHeaders.default)(accessToken),
    mode: 'cors',
    signal: controller.signal
  });
  clearTimeout(timeoutId);
  if (res.status >= 400 && res.status < 500) {
    if (res.status === 401) {
      // Token expired, notifying
      onTokenExpired === null || onTokenExpired === void 0 ? void 0 : onTokenExpired();
    } else if (res.status === 403) {
      // Session expired, reloading Web View
      reloadWebView();
    }
    const error = new _types.ChatGptError((0, _httpCodes.getStatusText)(res.status));
    error.statusCode = res.status;
    throw error;
  } else if (res.status >= 500) {
    const error = new _types.ChatGptError(`ChatGPTResponseServerError: ${res.status} ${res.statusText}`);
    error.statusCode = res.status;
    throw error;
  }
  const rawText = await res.text();
  const parsedData = (0, _parseStreamedGptResponse.default)(rawText);
  if (!parsedData) {
    throw new _types.ChatGptError('ChatGPTResponseError: Unable to parse response');
  }
  return parsedData;
}
function reloadWebView() {
  var _webview2;
  (_webview2 = webview) === null || _webview2 === void 0 ? void 0 : _webview2.reload();
}

/**
 * Removes the icon button in the top right corner of the webview screen when
 * ChatGPT is at full capacity
 */
async function removeThemeSwitcher() {
  var _webview3;
  // Apparently the button is not there yet after the page loads, so we wait a bit
  await (0, _wait.default)(200);
  const script = `
    (() => {
      const xpath = "//div[contains(text(),'ChatGPT is at capacity right now')]";
      const element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      if (element) {
        window.removeThemeSwitcher();
      }
    })();

    true;
  `;
  (_webview3 = webview) === null || _webview3 === void 0 ? void 0 : _webview3.injectJavaScript(script);
}

/**
 * Checks if ChatGPT servers are overloaded and the normal login page is not accessible
 */
function checkFullCapacity() {
  var _webview4;
  const script = `
    (() => {
      const xpath = "//div[contains(text(),'ChatGPT is at capacity right now')]";
      const element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      if (element) {
        window.removeThemeSwitcher();
        window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'CHAT_GPT_FULL_CAPACITY' }));
      }
    })();

    true;
  `;
  (_webview4 = webview) === null || _webview4 === void 0 ? void 0 : _webview4.injectJavaScript(script);
}

/**
 * Refreshes the webview and checks again if the login page is available
 */
async function retryLogin() {
  reloadWebView();
  // Waiting 3 seconds before checking again
  await (0, _wait.default)(3000);
  checkFullCapacity();
}
function navigateToLoginPage() {
  var _webview5;
  const script = `
   (() => {
      window.location.replace("${_constants.LOGIN_PAGE}");
   })();

   true;
  `;
  (_webview5 = webview) === null || _webview5 === void 0 ? void 0 : _webview5.injectJavaScript(script);
}
//# sourceMappingURL=index.js.map